...
async function getResources() {
    const resource = getCommodities()
        .then(function (commodities) {
            const result = Promise.all(commodities.map(function (commodity) {
...
async function fillUSDPrice(commodity) {
    return convertCurrency(commodity.price, "IDR", "USD")
        .then(function (price_usd) {
            try {
